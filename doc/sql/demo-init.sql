...
(
    ...
    `delay_time`  bigint       DEFAULT NULL COMMENT '延时消息的延时时间，单位：秒',
    `headers`     varchar(512) DEFAULT NULL COMMENT '消息头信息',
...
(
    ...
    `request_id`            varchar(64)  NOT NULL COMMENT '事件ID/消息ID,默认UUID,可自定义实现接口（RequestIdGenerator）获取消息的ID',
    `service_id`            varchar(100) NOT NULL COMMENT '消息监听器所属服务ID/服务名',
    `code`                  varchar(256) DEFAULT NULL COMMENT '消息编码',
    `type`                  int(1) DEFAULT NULL COMMENT '消息类型,1及时消息、2延时消息',
    `delay_time`            bigint       DEFAULT NULL COMMENT '延时时间，单位：秒',
    ...
    `exception_stack_trace` mediumtext COMMENT '异常堆栈信息',
    `create_time`           datetime     DEFAULT NULL COMMENT '创建时间',
